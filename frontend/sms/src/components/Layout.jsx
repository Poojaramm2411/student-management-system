...
export default function Layout() {
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Sidebar collapsed={collapsed} setCollapsed={setCollapsed} />
      {/*
        minWidth: 0 is the key fix here.
        Flex children default to min-width: auto, which stops this Box
        from shrinking below the width of its content (the table).
        When the sidebar expands and eats horizontal space, that caused
        the table columns (and the ₹ amounts) to get compressed/wrapped
        instead of the table scrolling horizontally.
        Setting minWidth: 0 lets this container shrink properly so the
        overflow-x:auto on .table-container (Table.css) can take over.
      */}
